import { Scene, BufferGeometry, ShaderMaterial, Points, BufferAttribute, Color, Material, IUniform } from "three";
...
export class Nodes {
  ...
  constructor(private scene: Scene, private pickingScene: Scene, private uniforms: { [uniform: string]: IUniform<any> }) {
  ...
  private createNodes = (pos: BufferAttribute, colorArr: Float32Array, reference: BufferAttribute) => {
    ...
    points.renderOrder = 999; // set points always on top
    return points;
  }
  private geneNodeColor = (group: number) => {
    if (!this.colorMap.has(group)) {
      this.colorMap.set(group, [Math.random(), Math.random(), Math.random()]);
    }
    return this.colorMap.get(group) || [0, 0, 0];
  }
  private genePickingNodeColor = (i: number) => {
    return new Color().setHex(i + 1);
  }
  private createNodesColor = (size: number, nodes: DataNode[]) => {
